...
//Instantiate models
var models = {};
['user', 'zone', 'audio', 'light', 'security', 'lock'].forEach(function(m){
  models[m] = sequelize.import(__dirname + '/../models/'+m)
});

//Archives
['energy', 'security', 'light', 'temperature', 'lock'].forEach(function(m){
        ...
        Audio.sync().success(function(){
          ...
          Light.drop().success(function(){
            Light.sync().success(function(){
              ...
              Security.drop().success(function(){
                Security.sync().success(function(){
                  Security.create()
                  
                  Lock.drop().success(function(){
                    Lock.sync().success(function(){
                      Lock.create()
                      
                      //MySQL Triggers
                      updateTrigger('lightLog', 'lights', ['lightId', 'level'], ['new.id', 'new.level'])
