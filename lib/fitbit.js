...
//Persist user's fitbit access token within the session
FitbitClient.prototype.persist = function(req, token){
  req.session.fitbit = token
}

FitbitClient.prototype.userRequest = function(action, req, fn){
  var tok;
  try { tok = this.serializer.parse(req.session.fitbit) } catch(e){}
  if(tok == undefined) fn({ error:'Missing Fitbit Token' })
  else{
