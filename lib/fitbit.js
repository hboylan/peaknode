...
FitbitClient.prototype._get = function(path, params, token, callback) {
  this.oauth.get(this.baseURI + path + '?' + query.stringify(params),
            token.oauth_token, token.oauth_token_secret, this._requestCallback(callback));
}

FitbitClient.prototype.post = function(path, params, token, callback) {
  this.oauth.post(this.baseURI + path, token.oauth_token, token.oauth_token_secret,
             params, null, this._requestCallback(callback));
}
...
FitbitClient.prototype.getAccessToken = function (req, res, callback) {
  ...
  if(req.cookies && req.cookies.fitbit_client)
    try { sess = this.serializer.parse(req.cookies.fitbit_client) } catch(E) { }
  ...
  if(has_token &&  has_secret) // Access token
    ...
    this.oauth.getOAuthRequestToken({oauth_callback: callbackURI},
      function (error, oauth_token, oauth_token_secret, oauth_authorize_url, additionalParameters) {
        if (error) return callback(error, null);
        res.cookie('fitbit_client', this.serializer.stringify({token_secret:oauth_token_secret}), { path: '/', httpOnly: false })
    })
}
