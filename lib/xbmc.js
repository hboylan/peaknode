...
function XBMC(config){
  this.connection, this.api, this.active,
  this.host = config.xbmc_host, this.port = config.xbmc_port;
  this.connect()
}

XBMC.prototype.connect = function(fn){
  if(this.active) this.connection.close(), this.api.close();
  this.connection = undefined, this.api = undefined;
  this.connection = new xbmc.TCPConnection({host:this.host, port:this.port, verbose:false}), this.api = new xbmc.XbmcApi;
